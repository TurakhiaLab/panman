...
utility::Node::Node(std::string id, double len){
    identifier = id;
    level = 1;
    branchLength = len;
    parent = nullptr;
...
utility::Node::Node(std::string id, Node* par, double len){
    identifier = id;
    branchLength = len;
    parent = par;
    level = par->level + 1;
...
utility::Tree::Tree(std::string newickString) {
    ...
    std::vector<std::queue<double>> branchLen (128);  // will be resized later if needed
    size_t level = 0;
    ...
    for (auto s: s1) {
        ...
        for (auto c: s) {
            if (c == ':') {
                ...
                stop = true;
                nc++;
                double len = (branch.size() > 0) ? std::stof(branch) : -1.0;
