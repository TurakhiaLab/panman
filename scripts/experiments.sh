...
for DEST_FILE in $DEST_FILES; do
    ...
    if [ "$ARCH" = "linux/amd64" ] || [ "$ARCH" = "x86-64" ]; then
        ...
        # Pangraph software that supports ARM architecture does not output the newick tree. Users can use Mashtree to do so, but it might differ from the one generated by Pangraph.    
        # Constructing PanGraph from the Raw Sequences
    ...
    if [[ $DEST_FILE == *"tb"* || $DEST_FILE == *"klebs"* || $DEST_FILE == *"ecoli"* ]]; then    
        panmanUtils -I panman/out.panman --fasta-aligned -o out
