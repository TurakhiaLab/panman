    ...
    for(size_t i = 0; i < root->nucMutation.size(); i++) {
        ...
        if(type < 3) {
                ...
                if(secondaryBlockId != -1) {
                    if(nucGapPosition != -1) {
                        for(int j = 0; j < len; j++) {
                            ...
                            newVal = panmanUtils::getSymbolFromCode(static_cast<int>(panmanUtils::packedMutationSymbolAt(root->nucMutation[i].nucs, j, alphabet)), alphabet);
            ...
            if(type == panmanUtils::NucMutationType::NSNPS) {
                // SNP Substitution
                newVal = panmanUtils::getSymbolFromCode(static_cast<int>(panmanUtils::singleMutationSymbol(root->nucMutation[i].nucs, alphabet)), alphabet);
                if(secondaryBlockId != -1) {
                    if(nucGapPosition != -1) {
...
void panmanUtils::Tree::printFASTAUltraFast(std::ostream& fout, bool aligned, bool rootSeq, const std::tuple< int, int, int, int >& panMATStart, const std::tuple< int, int, int, int >& panMATEnd, bool allIndex, int chrID) {
    ...
    // tbb::parallel_for_each(allNodes.begin(), allNodes.end(), [&](const std::pair<std::string, panmanUtils::Node*>& keyValue) {
        ...
        // std::cout << "consensusSeq_string: " << consensusSeq_string << std::endl;
